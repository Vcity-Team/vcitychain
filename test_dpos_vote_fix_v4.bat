@echo off
echo Testing DPoS Vote Fix v4 - Complete Fix with Enhanced Debugging...
echo.
...
echo.
echo Testing DPoS vote command with enhanced debugging...
...
echo 3. Store type information
echo 4. Transaction pool interface availability
echo 5. DPoS consensus engine information
echo 6. Transaction pool integration status
echo 7. DPoS state update status
echo.
echo Expected Results:
echo - Should see detailed store type and method information
echo - Should see "Store implements AddTx interface" message
echo - Should see "Transaction added to pool successfully" (if interface works)
echo - Should see consensus engine type and available methods
echo - Should see detailed debugging information for troubleshooting
echo.
echo Stopping node...
