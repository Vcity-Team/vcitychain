...
type DelegateRegistration struct {
	...
	Rank         int           `json:"rank"`         // 排名（按 votingPower 倒序排序后的序号，从1开始）
	// VotingPower 仅查询时从 StakeStore 填充，不写入 RegistrationStore。
	VotingPower *big.Int `json:"votingPower,omitempty"`
	// 冻结相关字段
	...
	// LegacyDepositContract 历史审计字段：老注册（tx.To=nil，保证金在 CREATE 合约地址）与候选人的对应关系。
	// 主网历史数据可能已有此字段；链上 DPOS+MIG 迁移已下线，节点不再写入或执行迁移。
