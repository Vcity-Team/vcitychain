...
func (s *StakeStore) GetValidatorsWithFilter(filterZeroVotingPower bool) (validator.AccountSet, error) {
	...
	// 按权重倒序排序，确保权重高的验证者排在前面（权重相同时按地址字节升序排序，确保与GetSortedValidatorsWithLimit一致）
	sort.Slice(validators, func(i, j int) bool {
		...
		// 如果权重相同，按地址字节升序排序（确保排序稳定，与GetSortedValidatorsWithLimit一致）
...
// MigrateCumulativeRewards 迁移历史奖励数据，计算并更新StakeInfo中的累计奖励
// 这个函数只需要在升级时运行一次
// 即使没有奖励记录也不会出错
func (s *StakeStore) MigrateCumulativeRewards(rewardStore *RewardStore, logger hclog.Logger) error {
	if s.db == nil {
	...
	if rewardStore == nil {
		if logger != nil {
			logger.Warn("⚠️ RewardStore为nil，跳过奖励迁移（可能是首次运行，没有奖励记录）")
		}
		return nil // 不返回错误，只是跳过
	}

	if logger != nil {
		logger.Info("🔄 开始迁移历史奖励数据到StakeInfo...")
	}

	// 第一步：检查rewards bucket是否存在
	hasRewards := false
	err := rewardStore.db.View(func(tx *bolt.Tx) error {
		rewardsBucket := tx.Bucket([]byte("rewards"))
		if rewardsBucket == nil {
			return nil // bucket不存在，说明没有奖励记录
		}

		// 检查是否有任何记录
		cursor := rewardsBucket.Cursor()
	...
	if err != nil {
		if logger != nil {
			logger.Warn("⚠️ 检查rewards bucket失败", "error", err)
		}
	...
	if !hasRewards {
		if logger != nil {
			logger.Info("ℹ️ 没有发现奖励记录，跳过迁移（这是正常的，如果系统从未发放过奖励）")
		}
	...
	err = s.db.View(func(tx *bolt.Tx) error {
		...
		if stakingBucket == nil {
			if logger != nil {
				logger.Info("ℹ️ StakingInfo bucket不存在，没有质押数据，跳过迁移")
			}
			return nil
		}
	...
	if err != nil {
		if logger != nil {
			logger.Warn("⚠️ 收集地址失败", "error", err)
		}
		return nil // 不阻断
	}
	...
	if len(addresses) == 0 {
		if logger != nil {
			logger.Info("ℹ️ 没有找到需要迁移的地址，跳过")
		}
		return nil
	}

	if logger != nil {
		logger.Info("📊 找到需要迁移的地址", "count", len(addresses))
	}
	...
	err = s.db.Update(func(tx *bolt.Tx) error {
		...
		for addr := range addresses {
			// 获取该地址的所有历史奖励（从epoch 1开始）
			summary, err := rewardStore.GetRewardSummary(addr.String(), 1, 999999)
			if err != nil {
				// 如果rewards bucket不存在，这是正常的（首次运行）
				if strings.Contains(err.Error(), "rewards bucket not found") {
					if logger != nil {
						logger.Debug("ℹ️ rewards bucket不存在，跳过", "address", addr.String())
					}
					skippedCount++
					continue
				}
				if logger != nil {
					logger.Warn("⚠️ 获取奖励汇总失败", "address", addr.String(), "error", err)
				}
				errorCount++
				continue // 继续处理下一个地址
			}
			...
			if summary == nil {
				if logger != nil {
					logger.Debug("ℹ️ 奖励汇总为nil，跳过", "address", addr.String())
				}
				skippedCount++
				continue
			}
			...
			// 检查是否有奖励
			if summary.TotalRewardWei == "" || summary.TotalRewardWei == "0" {
				if logger != nil {
					logger.Debug("ℹ️ 该地址没有奖励记录", "address", addr.String())
				}
				skippedCount++
				continue // 没有奖励，跳过
			}
			...
			// 解析累计奖励
			cumulativeReward, ok := new(big.Int).SetString(summary.TotalRewardWei, 10)
			if !ok || cumulativeReward.Sign() == 0 {
				if logger != nil {
					logger.Debug("ℹ️ 累计奖励为0或解析失败", "address", addr.String(), "totalReward", summary.TotalRewardWei)
				}
				skippedCount++
				continue
			}
			...
			// 查找并更新StakeInfo
			// 注意：StakeInfo可能使用复合key，需要遍历所有相关记录
			cursor := stakingBucket.Cursor()
			found := false
			...
			for key, value := cursor.First(); key != nil; key, value = cursor.Next() {
				...
				if err := json.Unmarshal(value, &stakeInfo); err != nil {
					if logger != nil {
						logger.Warn("⚠️ 解析StakeInfo失败", "address", addr.String(), "error", err)
					}
					continue
				}

				// 如果已经有累计奖励且不为0，跳过（避免重复计算）
				if stakeInfo.Rewards != nil && stakeInfo.Rewards.Sign() > 0 {
					if logger != nil {
						logger.Debug("ℹ️ 该StakeInfo已有累计奖励，跳过",
							"address", addr.String(),
							"existingReward", stakeInfo.Rewards.String())
					}
					found = true
					continue
				}
				...
				// 更新累计奖励
				stakeInfo.Rewards = new(big.Int).Set(cumulativeReward)

				// 保存回数据库
				updatedData, err := json.Marshal(stakeInfo)
				if err != nil {
					if logger != nil {
						logger.Warn("⚠️ 序列化StakeInfo失败", "address", addr.String(), "error", err)
					}
					continue
				}
				...
				if err := stakingBucket.Put(key, updatedData); err != nil {
					if logger != nil {
						logger.Warn("⚠️ 更新StakeInfo失败", "address", addr.String(), "error", err)
					}
					continue
				}
				...
				if logger != nil {
					logger.Info("✅ 更新累计奖励",
						"address", addr.String(),
						"cumulativeReward", cumulativeReward.String(),
			...
			if !found {
				if logger != nil {
					logger.Debug("ℹ️ 未找到该地址的StakeInfo记录", "address", addr.String())
				}
				skippedCount++
			}
		}
	...
	if err != nil {
		if logger != nil {
			logger.Error("❌ 更新StakeInfo失败", "error", err)
		}
		return fmt.Errorf("failed to update staking info: %w", err)
	}

	if logger != nil {
		logger.Info("✅ 历史奖励数据迁移完成",
