...
type runtimeConfig struct {
	...
	// SyncLagRestartBlocks：本地落后于网络至少该块数（lag>=）才考虑方案 B。
	...
	// BootstrapRPC：可选 JSON-RPC（与创世 dpos_bootstrap_rpc 同源）；用于 eth_blockNumber 作为 canonical 链尖参考，与 gossip 合成门禁水位。
	BootstrapRPC string

	// MaxGossipLeadOverBootstrapRPC：gossip 宣称相对 eth_blockNumber 最多可高的块数（默认 5）；超过部分不参与抬门禁。0 表示用默认 5。
	MaxGossipLeadOverBootstrapRPC uint64
}
...
func (c *consensusRuntime) calculateCommitEpochInput(
	...
	*contractsapi.DistributeRewardForRewardPoolFn, error) {
	...
	blockExtra, err := GetDposExtra(currentBlock.ExtraData)
	if err != nil {
		return nil, nil, err
	}
...
func (c *consensusRuntime) getFirstBlockOfEpoch(epochNumber uint64, latestHeader *types.Header) (uint64, error) {
	...
	blockExtra, err := GetDposExtra(latestHeader.ExtraData)
	if err != nil {
		return 0, err
	}
