...
func TestDPoSConfig(t *testing.T) {
	...
	assert.Equal(t, 15*time.Second, config.BlockTime)
...
func TestDelegateUpdate(t *testing.T) {
	...
	initialDelegates := []*validator.ValidatorMetadata{
		{Address: types.Address{1}, VotingPower: big.NewInt(1000), IsActive: true},
		{Address: types.Address{2}, VotingPower: big.NewInt(2000), IsActive: true},
		{Address: types.Address{3}, VotingPower: big.NewInt(3000), IsActive: true},
	}
	...
	assert.Equal(t, types.Address{3}, dpos.delegates[0].Address) // 最高投票权重
	assert.Equal(t, types.Address{2}, dpos.delegates[1].Address)
	assert.Equal(t, types.Address{1}, dpos.delegates[2].Address) // 最低投票权重
}
...
func TestRewardCalculation(t *testing.T) {
	...
	// 设置测试数据
	dpos.voters[types.Address{1}] = &VoterInfo{
		Address:     types.Address{1},
		VotingPower: big.NewInt(1000),
	}
	dpos.voters[types.Address{2}] = &VoterInfo{
	...
	// 计算奖励
	reward1 := dpos.calculateReward(types.Address{1})
	reward2 := dpos.calculateReward(types.Address{2})
...
func TestBatchProcessing(t *testing.T) {
	...
	votes := []*VoteMessage{
		...
		{
			Voter:     types.Address{3},
			Delegate:  types.Address{11},
	...
	// 验证投票者信息
	assert.Equal(t, big.NewInt(1000000000000000000), dpos.voters[types.Address{1}].VotingPower)
	assert.Equal(t, big.NewInt(2000000000000000000), dpos.voters[types.Address{2}].VotingPower)
	assert.Equal(t, big.NewInt(3000000000000000000), dpos.voters[types.Address{3}].VotingPower)
	
	// 验证受托人投票权重
	delegate10Found := false
	delegate11Found := false
	for _, del := range dpos.delegates {
		if del.Address == types.Address{10} {
		...
		if del.Address == types.Address{11} {
...
func TestMetrics(t *testing.T) {
	...
	// 记录投票
	start := time.Now()
	collector.RecordVote(types.Address{1}, types.Address{2}, big.NewInt(1000), time.Since(start))
	
	// 记录受托人更新
	collector.RecordDelegateUpdate([]types.Address{types.Address{1}, types.Address{2}}, 100*time.Millisecond)
	
	// 记录区块生产
	collector.RecordBlockProduction(100, 15*time.Second, big.NewInt(1000000))
...
func BenchmarkVoteProcessing(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		vote := &VoteMessage{
			Voter:     types.Address{byte(i % 100)},
			Delegate:  types.Address{byte(i % 50)},
			Amount:    big.NewInt(1000000000000000000),
			Round:     uint64(i),
			Timestamp: uint64(time.Now().Unix()),
			Signature: []byte("test-signature"),
...
func BenchmarkBatchProcessing(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		for j := 0; j < 100; j++ {
			votes[j] = &VoteMessage{
				Voter:     types.Address{byte((i + j) % 100)},
				Delegate:  types.Address{byte((i + j) % 50)},
				Amount:    big.NewInt(1000000000000000000),
				Round:     uint64(i),
				Timestamp: uint64(time.Now().Unix()),
				Signature: []byte("test-signature"),
...
func createTestDPoS(t interface{}) *DPoS {
	...
	// 初始化受托人
	dpos.delegates = []*validator.ValidatorMetadata{
		{Address: types.Address{10}, VotingPower: big.NewInt(0), IsActive: true},
		{Address: types.Address{11}, VotingPower: big.NewInt(0), IsActive: true},
	}
...
func TestIntegration(t *testing.T) {
	...
	// 1. 处理多个投票
	votes := []*VoteMessage{
		{Voter: types.Address{1}, Delegate: types.Address{10}, Amount: big.NewInt(1000), Round: 1, Timestamp: uint64(time.Now().Unix()), Signature: []byte("sig1")},
		{Voter: types.Address{2}, Delegate: types.Address{10}, Amount: big.NewInt(2000), Round: 1, Timestamp: uint64(time.Now().Unix()), Signature: []byte("sig2")},
		{Voter: types.Address{3}, Delegate: types.Address{11}, Amount: big.NewInt(3000), Round: 1, Timestamp: uint64(time.Now().Unix()), Signature: []byte("sig3")},
	}
	...
	// 2. 验证投票者状态
	assert.Equal(t, big.NewInt(1000), dpos.voters[types.Address{1}].VotingPower)
	assert.Equal(t, big.NewInt(2000), dpos.voters[types.Address{2}].VotingPower)
	assert.Equal(t, big.NewInt(3000), dpos.voters[types.Address{3}].VotingPower)
	...
	reward1 := dpos.calculateReward(types.Address{1})
	reward2 := dpos.calculateReward(types.Address{2})
	reward3 := dpos.calculateReward(types.Address{3})
