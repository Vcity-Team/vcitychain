...
func newCheckpointManager(key ethgo.Key, checkpointOffset uint64,
	...
	blockchain blockchainBackend, backend dposBackend, logger hclog.Logger,
...
func (c *checkpointManager) encodeAndSendCheckpoint(header *types.Header, extra *Extra, isEndOfEpoch bool) error {
	...
	if isEndOfEpoch {
		var err error
		nextEpochValidators, err = c.consensusBackend.GetDelegates(header.Number, nil)
