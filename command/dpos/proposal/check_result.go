...
// GetCheckResultCommand 获取检查结果命令
func GetCheckResultCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "check-result",
		Short: "检查提案投票结果",
		Long:  "检查指定提案的投票结果是否通过",
		RunE:  runCheckResult,
	}
...
func runCheckResult(cmd *cobra.Command, args []string) error {
	// 获取参数
	proposalID, _ := cmd.Flags().GetString("proposal-id")

	// 调用RPC
	response, err := callCheckResultRPC(proposalID)
	if err != nil {
...
func callCheckResultRPC(proposalID string) (*CheckResultResponse, error) {
	// 构建JSON-RPC请求
