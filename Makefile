...
build: check-go check-git
	...
	go build -o vcitychain -ldflags="\
    	-X 'github.com/Vcity-Team/vcitychain/versioning.Version=$(VERSION)' \
		-X 'github.com/Vcity-Team/vcitychain/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/Vcity-Team/vcitychain/versioning.Branch=$(BRANCH)'\
		-X 'github.com/Vcity-Team/vcitychain/versioning.BuildTime=$(TIME)'" \
	main.go
...
.PHONY: test-e2e
test-e2e: check-go
	go build -race -o artifacts/vcitychain .
	env EDGE_BINARY=${PWD}/artifacts/vcitychain go test -v -timeout=30m ./e2e/...

.PHONY: test-e2e-polybft
test-e2e-polybft: check-go
	go build -o artifacts/vcitychain .
	env EDGE_BINARY=${PWD}/artifacts/vcitychain E2E_TESTS=true E2E_LOGS=true \
	go test -v -timeout=1h30m ./e2e-polybft/e2e/...

.PHONY: test-property-polybft
test-property-polybft: check-go
	go build -o artifacts/vcitychain .
	env EDGE_BINARY=${PWD}/artifacts/vcitychain E2E_TESTS=true E2E_LOGS=true go test -v -timeout=30m ./e2e-polybft/property/... \
	-rapid.checks=10
